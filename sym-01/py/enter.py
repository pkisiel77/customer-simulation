...
def enter(czas, nr_zdarz, typ_kl, nr_ev):
    ...
    if rand < p_wej:
        ...
        time = time - (T_exp*log(1-rand))
        typ = 2  # ekspozycja
        # tb(nr_ev)=czas;
        nr_zdarz = rej_evt(czas, typ, nr_zd_obs, typ_kl,
                           nr_ev)  # rejestracja zdarzenia
    else:
        nr_zdarz = del_evt(czas, nr_zdarz, nr_zd_obs, typ_kl,
                           typ, nr_ev)  # usuwamy klienta
    return 0
